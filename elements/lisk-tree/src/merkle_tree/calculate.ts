...
import { buildBranch, createNewBranchNode, createNewLeafNode, generateNode, getBinaryString,
    getHeight, getLocationFromIndex, getPairLocation, getParentLocation } from "./utils";
...
export const calculatePathNodes = (
    ...
    while (locations[0].layerIndex as number < getHeight(size) - 1) {
        ...
        if (tree[pairBinaryIndex] && tree[pairBinaryIndex].hash !== currentSiblingHash && currentNode.hash !== currentSiblingHash) {
...
export const calculateRootFromUpdateData = (
	...
	const calculatedTree = calculatePathNodes(updateHashes, size, indexes as unknown as ReadonlyArray<number>, siblingHashes as unknown as ReadonlyArray<Buffer>);
