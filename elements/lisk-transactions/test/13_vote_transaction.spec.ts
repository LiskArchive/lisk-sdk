...
describe('Vote transaction', () => {
	...
	beforeEach(() => {
		{
			const buffer = Buffer.from(validUpvoteTransactionScenario.output.transaction, 'base64');
		...
		{
			const buffer = Buffer.from(validDownvoteTransactionScenario.output.transaction, 'base64');
			const id = hash(buffer);
			const decodedBaseTransaction = codec.decode<BaseTransaction>(
				BaseTransaction.BASE_SCHEMA,
				buffer,
		...
		{
			const buffer = Buffer.from(fixtures.testCases[2].output.transaction, 'base64');
			const id = hash(buffer);
			const decodedBaseTransaction = codec.decode<BaseTransaction>(
				BaseTransaction.BASE_SCHEMA,
				buffer,
	...
	describe('validateAsset', () => {
		...
		describe('when asset.votes includes more than 10 positive votes', () => {
			it('should return errors', () => {
				...
				expect(status).toBe(Status.FAIL);
				expect(errors).toHaveLength(1);
				expect(errors[0].message).toInclude('Upvote can only be casted upto 10');
			});
		});
		...
		describe('when asset.votes includes more than 10 negative votes', () => {
			it('should return errors', () => {
				...
				expect(status).toBe(Status.FAIL);
				expect(errors).toHaveLength(1);
				expect(errors[0].message).toInclude('Downvote can only be casted upto 10');
			});
		});
		...
		describe('when asset.votes includes duplicate delegates within positive amount', () => {
			it('should return errors', () => {
				...
				(tx.asset as any).votes[9] = {
					delegateAddress: decodedUpvoteTransaction.asset.votes[0].delegateAddress,
		...
		describe('when asset.votes includes zero amount', () => {
			it('should return errors', () => {
				...
				(tx.asset as any).votes[0] = {
					delegateAddress: decodedMixedvoteTransaction.asset.votes[0].delegateAddress,
					amount: BigInt(0),
				};
				const { errors, status } = tx.validate();
				expect(status).toBe(Status.FAIL);
		...
		describe('when asset.votes includes amount which is not multiple of 10 * 10^8', () => {
			it('should return errors', () => {
				...
				const { errors, status } = tx.validate();
				expect(status).toBe(Status.FAIL);
				expect(errors[0].message).toInclude('Amount should be multiple of 10 * 10^8');
			});
		});
	});
	...
	describe('applyAsset', () => {
		...
		describe('when asset.votes contain positive amount which makes account.votes to be 10 entries', () => {
			...
			beforeEach(async () => {
				...
				const sender = defaultAccount({
					...
					address: Buffer.from(validUpvoteTransactionScenario.input.account.address, 'base64'),
				...
				store = new StateStoreMock(
					[
						sender,
						...validUpvoteTransactionScenario.input.delegates.map((delegate, i) =>
							defaultAccount({
			...
			it('should make account to have correct balance', async () => {
				...
				const sender = await store.account.get<AccountAsset>(
					Buffer.from(validUpvoteTransactionScenario.input.account.address, 'base64'),
				);
			...
			it('should make upvoted delegate account to have correct totalVotesReceived', async () => {
				...
				for (const vote of decodedUpvoteTransaction.asset.votes) {
					...
					if (
						vote.delegateAddress.equals(decodedUpvoteTransaction.asset.votes[0].delegateAddress)
					) {
			...
			it('should update vote object when it exists before and create if it does not exist', async () => {
				...
				for (const vote of sender.asset.sentVotes) {
					...
					if (
						...
						expect(vote.amount.toString()).toEqual(BigInt(relatedVote.amount).toString());
					}
				}
			});
		});
		...
		describe('when asset.votes contain negative amount which makes account.votes to be 0 entries', () => {
			...
			beforeEach(async () => {
				...
				const totalSpending = BigInt(decodedDownvoteTransaction.fee) + minBalance;
				const sender = defaultAccount({
					nonce: BigInt(decodedDownvoteTransaction.nonce),
					address: Buffer.from(validDownvoteTransactionScenario.input.account.address, 'base64'),
					balance: totalSpending,
					asset: {
						sentVotes: [
			...
			it('should not change account balance', async () => {
				await tx.apply(store);
				const sender = await store.account.get<AccountAsset>(
					Buffer.from(validDownvoteTransactionScenario.input.account.address, 'base64'),
				);
				expect(sender.balance.toString()).toEqual(minBalance.toString());
			});
			...
			it('should update vote which has non-zero amount', async () => {
				...
				expect(sender.asset.sentVotes[0].amount.toString()).toEqual(originalVotes.toString());
			});
			...
			it('should make account to have correct unlocking', async () => {
				...
				for (const unlock of sender.asset.unlocking) {
					expect(unlock.unvoteHeight).toEqual(store.chain.lastBlockHeader.height + 1);
			...
			it('should order account.unlocking', async () => {
				...
				const expectedUnlock = decodedDownvoteTransaction.asset.votes.map(vote => ({
					delegateAddress: vote.delegateAddress,
					amount: BigInt(-1) * BigInt(vote.amount),
					unvoteHeight: store.chain.lastBlockHeader.height + 1,
				}));
				expectedUnlock.sort((a, b) => a.delegateAddress.compare(b.delegateAddress));
			...
			it('should make downvoted delegate account to have correct totalVotesReceived', async () => {
				...
				for (const vote of decodedDownvoteTransaction.asset.votes) {
					const delegate = await store.account.get<AccountAsset>(vote.delegateAddress);
					if (
						vote.delegateAddress.equals(decodedDownvoteTransaction.asset.votes[0].delegateAddress)
					) {
						expect(delegate.asset.delegate.totalVotesReceived.toString()).toEqual(
							originalVotes.toString(),
						);
					} else {
						expect(delegate.asset.delegate.totalVotesReceived.toString()).toEqual('0');
					}
				}
			});
		});
		...
		describe('when asset.votes contain negative and positive amount which makes account.votes to be 10 entries', () => {
			beforeEach(() => {
				...
				const sender = defaultAccount({
					...
					address: Buffer.from(validMixvoteTransactionScenario.input.account.address, 'base64'),
					balance: totalSpending,
					asset: {
						sentVotes: [
			...
			it('should make account to have correct balance', async () => {
				await tx.apply(store);
				const sender = await store.account.get<AccountAsset>(
					Buffer.from(validMixvoteTransactionScenario.input.account.address, 'base64'),
				);
				expect(sender.balance.toString()).toEqual(minBalance.toString());
			});
			...
			it('should order account.votes', async () => {
				...
				const senderVotesCopy = sender.asset.sentVotes.slice(0);
				senderVotesCopy.sort((a, b) => a.delegateAddress.compare(b.delegateAddress));
				expect(sender.asset.sentVotes).toStrictEqual(senderVotesCopy);
			});
			...
			it('should order account.unlocking', async () => {
				...
				senderUnlockingCopy.sort((a, b) => a.delegateAddress.compare(b.delegateAddress));
		...
		describe('given asset.votes contain invalid data', () => {
			...
			describe('when asset.votes contain delegate address which is not registered', () => {
				it('should return errors', async () => {
					...
					expect(status).toBe(Status.FAIL);
					expect(errors).toHaveLength(1);
					expect(errors[0].message).toInclude('Voted delegate is not registered');
				});
			});

			describe('when asset.votes positive amount makese account.votes entries more than 10', () => {
				it('should return errors', async () => {
					const invalidSender = await store.account.get<AccountAsset>(sender.address);
			...
			describe('when the last asset.votes amount makes sender not having sufficient balance', () => {
				it('should return errors', async () => {
					...
					expect(status).toBe(Status.FAIL);
					expect(errors).toHaveLength(1);
					expect(errors[0].message).toInclude('Account does not have enough minimum remaining');
				});
			});
			...
			describe('when asset.votes negative amount exceeds the previously voted amount', () => {
				it('should return errors', async () => {
					...
					expect(errors[0].message).toInclude('Cannot downvote more than upvoted');
				});
			});
		});
		...
		describe('when asset.votes contains self-vote', () => {
			...
			beforeEach(() => {
				...
				tx.sign(
					Buffer.from(validMixvoteTransactionScenario.input.networkIdentifier, 'base64'),
			...
			it('should update votes and totalVotesReceived', async () => {
				...
				expect(updatedSender.asset.delegate.totalVotesReceived.toString()).toEqual(
					voteAmount.toString(),
				);
		...
		describe('when asset.votes contains self-downvote', () => {
			...
			it('should update votes, totalVotesReceived and unlocking', async () => {
				...
				expect(updatedSender.asset.delegate.totalVotesReceived.toString()).toEqual('0');
				...
				expect(updatedSender.balance.toString()).toEqual((senderBalnce - tx.fee).toString());
			});
		});
	});
});
