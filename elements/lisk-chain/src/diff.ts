	...
	while (pointerMin < pointerMid) {
		if (
			buffer1
				...
				.equals(buffer2.slice(buffer2.length - pointerMid, buffer2.length - pointerEnd))
		) {
			pointerMin = pointerMid;
 ...
 */
export const calculateDiff = (initial: Buffer, final: Buffer): DiffHistory[] => {
	...
	const commonSuffix = diffCommonSuffix(strippedPrefixInitial, strippedPrefixFinal);
	const strippedInitial = strippedPrefixInitial.slice(
		0,
		strippedPrefixInitial.length - commonSuffix,
	);
	const strippedFinal = strippedPrefixFinal.slice(0, strippedPrefixFinal.length - commonSuffix);
	const longDiff = diffAlgo(strippedInitial, strippedFinal);

	// Add common prefix to the reduced array in the start
	const reducedDiff = commonPrefix > 0 ? [{ code: '=', line: commonPrefix }] : [];
	let count = 0;
...
export const undo = (finalBuffer: Buffer, diffArray: DiffHistory[]): Buffer => {
	...
	for (const { code, line } of diffArray.reverse()) {
		if (code === '=') {
			const unchangedBytes = finalBytes.slice(finalBytes.length - line, finalBytes.length);
