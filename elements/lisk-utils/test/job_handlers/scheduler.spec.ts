 ...
 *
 */
import { Scheduler } from '../../src/job_handlers/scheduler';
...
describe('Scheduler', () => {
	...
	describe('#constructor', () => {
		it('should return a job instance', () => {
			expect(new Scheduler(jobStub, interval)).toBeInstanceOf(Scheduler);
		});
	});

	describe('#start', () => {
		let job: Scheduler<number>;

		beforeEach(() => {
			job = new Scheduler(jobStub, interval);
			jest.useFakeTimers();
		});
