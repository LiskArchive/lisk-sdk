...
describe('Integration tests for P2P library', () => {
	...
	describe('Connected network: Minimum peer discovery threshold is set to 1', () => {
		beforeEach(async () => {
			p2pNodeList = [...new Array(NETWORK_PEER_COUNT).keys()].map(index => {
				// Each node will have the previous node in the sequence as a seed peer except the first node.
				const seedPeers =
		...
		it('should return list of peers with at most the minimum discovery threshold', async () => {
			...
			expect(newPeers.length).to.eql(1);
		});
	});

	describe('Connected network: Maximum peer discovery response size is set to 3', () => {
		beforeEach(async () => {
			p2pNodeList = [...new Array(NETWORK_PEER_COUNT).keys()].map(index => {
				// Each node will have the previous node in the sequence as a seed peer except the first node.
				const seedPeers =
				...
				return new P2P({
					...
					maxOutboundConnections: DEFAULT_MAX_OUTBOUND_CONNECTIONS,
					maxInboundConnections: DEFAULT_MAX_INBOUND_CONNECTIONS,
					maximumPeerDiscoveryResponseSize: 3,
					nodeInfo: {
						wsPort: nodePort,
						nethash:
							'da3ed6a45429278bac2666961289ca17ad86595d33b31037615d4b8e8f158bba',
		...
		it('should return list of peers with less than maximum discovery response size', async () => {
			const firstP2PNode = p2pNodeList[0];
			const { newPeers } = firstP2PNode.getNetworkStatus();
			expect(newPeers.length).to.be.lessThan(3);
		});
	});
});
