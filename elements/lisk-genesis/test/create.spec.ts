...
import { createGenesisBlock, getGenesisBlockJSON } from '../src';
...
describe('create', () => {
	...
	describe('getGenesisBlockJSON', () => {
		it('should generate the JSON object for the given genesis block', () => {
			// Arrange
			const genesisBlock = createGenesisBlock(validGenesisBlockParams);
			...
			const genesisBlockJSON = getGenesisBlockJSON({
				genesisBlock,
				accountAssetSchemas: validGenesisBlockParams.accountAssetSchemas,
			});

			// Assert
			expect(genesisBlockJSON).toMatchSnapshot();
		});
	});
});
