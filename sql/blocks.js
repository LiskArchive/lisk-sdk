  ...
  aggregateBlocksReward: function (params) {
    return [
      'WITH borders as (SELECT',
        ...
        '(SELECT (CAST(b.height / 101 AS INTEGER) + (CASE WHEN b.height % 101 > 0 THEN 1 ELSE 0 END)) FROM blocks b',
          (params.end !== undefined ? ' WHERE b.height <= ${end}' : ''),
          'ORDER BY b.height DESC LIMIT 1) as max',
      '),',
      'r as (SELECT DISTINCT ',
        '(CAST(b.height / 101 AS INTEGER) + (CASE WHEN b.height % 101 > 0 THEN 1 ELSE 0 END)) AS round',
        'FROM blocks b WHERE b."generatorPublicKey" = DECODE (${generatorPublicKey}, \'hex\')),',
      're as (SELECT r.round as round, ((r.round-1)*101)+1 as min, r.round*101 as max',
      ...
      'sum_min as (SELECT',
        ...
        (params.start !== undefined ? 'AND b.height >= ${start}' : ''),
      '),',
      'sum_max as (SELECT',
        'sum(CASE WHEN b."generatorPublicKey" = DECODE (${generatorPublicKey}, \'hex\') THEN b.reward ELSE 0 END) AS rewards,',
        'sum(CASE WHEN b."generatorPublicKey" = DECODE (${generatorPublicKey}, \'hex\') THEN 1 ELSE 0 END) AS blocks',
        ...
        (params.end !== undefined ? 'AND b.height <= ${end}' : ''),
      '),',
