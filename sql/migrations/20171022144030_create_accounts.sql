...
CREATE OR REPLACE FUNCTION protect_accounts_balance() RETURNS TRIGGER LANGUAGE PLPGSQL AS $$
	...
	BEGIN
		...
		INTO
			result
		FROM transactions
		WHERE block_id = genesis_block_id
		AND type = 0
		AND sender_address = NEW.address
...
-- Create function for rolling back public_key to a NULL state
CREATE OR REPLACE FUNCTION public_key_rollback() RETURNS TRIGGER LANGUAGE PLPGSQL AS $function$
	BEGIN
...
-- Create trigger for conditionally rolling back public_key state when updating accounts
...
-- Create function for validating balances on the accounts table against the blockchain
CREATE FUNCTION validate_accounts_balances() RETURNS TABLE(address VARCHAR(22), public_key TEXT, delegate VARCHAR(20), blockchain BIGINT, memory BIGINT, diff BIGINT) LANGUAGE PLPGSQL AS $$
	BEGIN
		RETURN QUERY
			WITH balances AS (
				(
			...
			SELECT
				m.address,
				ENCODE(m.public_key, 'hex') AS public_key,
				d.name,
				a.balance::BIGINT AS blockchain,
				m.balance::BIGINT AS memory,
				(m.balance-a.balance)::BIGINT AS diff
			FROM (
				accounts a
...
-- Create function for reverting any changes to immutable columns on an account
-- TODO: Rewrite function for new accounts table and create a new trigger
CREATE OR REPLACE FUNCTION revert_mem_account() RETURNS TRIGGER LANGUAGE PLPGSQL AS $function$
	BEGIN
