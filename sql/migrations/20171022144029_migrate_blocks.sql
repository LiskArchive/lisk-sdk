...
-- Alter sequences, indexes, constraints
ALTER SEQUENCE "blocks_rowId_seq" RENAME TO seq_blocks_row_id;
...
ALTER TABLE blocks DROP CONSTRAINT "blocks_previousBlock_fkey";
ALTER TABLE blocks ADD CONSTRAINT fkey_blocks_previous_block_id_fkey FOREIGN KEY(previous_block_id) REFERENCES blocks(block_id) ON DELETE SET NULL;
...
-- Create function for calculating block reward from a given block height
CREATE OR REPLACE FUNCTION calculate_block_reward(block_height integer) RETURNS bigint LANGUAGE PLPGSQL IMMUTABLE AS $function$
	DECLARE
		r block_rewards;
		mile int;
	BEGIN
		IF block_height IS NULL OR block_height <= 0 THEN
			RETURN NULL;
		END IF;
...
-- Create function for calculating supply from a given block height
CREATE OR REPLACE FUNCTION calculate_supply(block_height integer) RETURNS bigint LANGUAGE PLPGSQL IMMUTABLE AS $function$
	DECLARE
		r block_rewards;
		mile int;
	BEGIN
...
-- Create function for testing supply calculation for a given range of block heights, against a given expected reward
CREATE OR REPLACE FUNCTION calculate_supply_test(height_start integer, height_end integer, expected_reward bigint) RETURNS boolean LANGUAGE PLPGSQL IMMUTABLE AS $function$
	DECLARE
		supply bigint;
		prev_supply bigint;
	BEGIN
		SELECT
			calculate_supply(height_start - 1)
		INTO
			prev_supply;

		FOR height IN height_start..height_end LOOP
			SELECT
				calculate_supply(height)
			INTO
				supply;
...
-- Create function for returning block rewards info
CREATE OR REPLACE FUNCTION get_block_rewards() RETURNS block_rewards LANGUAGE PLPGSQL IMMUTABLE AS $function$
