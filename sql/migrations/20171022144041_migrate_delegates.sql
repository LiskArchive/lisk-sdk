...
-- Create function for updating delegate ranks according to 'voters_balance'
CREATE OR REPLACE FUNCTION delegates_rank_update() RETURNS TABLE(updated integer) LANGUAGE PLPGSQL AS $function$
	BEGIN
		RETURN QUERY WITH new AS (
			SELECT
				row_number() OVER(ORDER BY voters_balance DESC, public_key ASC) AS rank,
				transaction_id
			FROM delegates
		),
		updated AS (
...
-- Create function that will execute 'delegates_rank_update'
CREATE OR REPLACE FUNCTION delegate_change_ranks_update() RETURNS trigger LANGUAGE plpgsql AS $function$
	BEGIN
		PERFORM delegates_rank_update();
	RETURN NULL;
END $function$;

-- Create function for updating delegate forged block counts
CREATE OR REPLACE FUNCTION delegates_forged_blocks_count_update() RETURNS trigger LANGUAGE plpgsql AS $function$
	BEGIN
...
-- Create trigger that will execute 'delegates_forged_blocks_cnt_update' after insertion or deletion of a block
...
-- Create function for updating delegate voters balances according to received votes
CREATE OR REPLACE FUNCTION delegates_voters_balance_update() RETURNS TABLE(updated integer) LANGUAGE plpgsql AS $function$
	BEGIN
		RETURN QUERY
			WITH last_round AS (
				SELECT
					(CASE WHEN height < 101 THEN 1 ELSE height END) AS height
				FROM blocks
			...
			voters AS (
				SELECT
					DISTINCT ON (voter_address) voter_address
				FROM votes_details
			),
			balances AS (
				(
					SELECT
						UPPER(sender_address) AS address,
				...
				UNION ALL (
					SELECT
						UPPER(recipient_address) AS address,
			...
			updated AS (
				...
				WHERE delegates.public_key = dd.public_key
				RETURNING 1
			)
			SELECT
				COUNT(1)::INT
...
-- Create function for updating delegate voter counts according to received votes
CREATE OR REPLACE FUNCTION delegates_voters_count_update() RETURNS TABLE(updated integer) LANGUAGE plpgsql AS $function$
	BEGIN
		RETURN QUERY
			WITH last_round AS (
				SELECT
...
-- Create function that will execute delegate functions or notifications dependent on whether a block is being inserted or deleted
CREATE OR REPLACE FUNCTION delegates_update_on_block() RETURNS trigger LANGUAGE plpgsql AS $function$
	BEGIN
		...
		END IF;
		IF (TG_OP = 'INSERT') THEN
			PERFORM pg_notify('round-closed',
				json_build_object('round',
					CEIL((NEW.height + 1) / 101::float)::int,
					'list',
		...
		ELSIF (TG_OP = 'DELETE') THEN
			PERFORM pg_notify('round-reopened',
				json_build_object('round',
					CEIL((OLD.height) / 101::float)::int,
					'list',
					generate_delegates_list(
						CEIL((OLD.height) / 101::float)::int,
						ARRAY(
...
-- Create function for generating a delegate list for the given round and delegates
CREATE OR REPLACE FUNCTION generate_delegates_list(round integer, delegates text[]) RETURNS text[] LANGUAGE plpgsql IMMUTABLE AS $function$
	DECLARE
		i int;
		x int;
		n int;
...
-- Create a function for getting the generated delegates list
CREATE OR REPLACE FUNCTION get_delegates_list() RETURNS text[] LANGUAGE plpgsql AS $function$
	DECLARE
		list text[];
	BEGIN
		SELECT generate_delegates_list(
			(
				SELECT
					CEIL((height + 1) / 101::float)::int AS round
				FROM blocks
				ORDER BY height DESC LIMIT 1
			),
			ARRAY(
