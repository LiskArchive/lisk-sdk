...
module.exports = function(
	...
	describe('@p2p : peer Protocol Version', () => {
		describe('when a node with protocol = 0.0 (invalid) tries to join the network', () => {
			const originalProtocolVersion = __testContext.config.protocolVersion;
			...
			before(async () => {
				...
				pm2NetworkConfig.apps[0].env.PROTOCOL_VERSION = '0.0';
				await fs_writeFile(
					`${__dirname}/../../pm2.network.json`,
					JSON.stringify(pm2NetworkConfig, null, 4)
				);

				await network.reloadNode('node_0', true, true);
				await network.waitForBlocksOnNode('node_1', 4);
			});

			after(async () => {
				pm2NetworkConfig.apps[0].env.PROTOCOL_VERSION = originalProtocolVersion; // '1.0'
				await fs_writeFile(
					`${__dirname}/../../pm2.network.json`,
					JSON.stringify(pm2NetworkConfig, null, 4)
				);
			...
			it(`there should be ${EXPECTED_TOTAL_CONNECTIONS_AFTER_STARTING_INCOMPATIBLE_PEER} established connections from 500[0-9] ports`, () => {
				return utils
