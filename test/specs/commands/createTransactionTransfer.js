					...
					Given('an empty options object', given.anEmptyOptionsObject, () => {
						Given('an address "13356260975429434553L"', given.anAddress, () => {
							Given('an invalid amount "abc"', given.anInvalidAmount, () => {
								When('the action is called with the amount, the address and the options', when.theActionIsCalledWithTheAmountTheAddressAndTheOptions, () => {
									Then('it should reject with message "Amount must be a number with no more than 8 decimal places."', then.itShouldRejectWithMessage);
								});
							});
						...
						Given('an amount "100.123"', given.anAmount, () => {
							Given('an invalid address "1234567890LL"', given.anInvalidAddress, () => {
								When('the action is called with the amount, the address and the options', when.theActionIsCalledWithTheAmountTheAddressAndTheOptions, () => {
									Then('it should reject with message "1234567890LL is not a valid address."', then.itShouldRejectWithMessage);
								});
							});
							Given('an address "13356260975429434553L"', given.anAddress, () => {
								...
								Given('the passphrase can be retrieved from its source', given.thePassphraseCanBeRetrievedFromItsSource, () => {
									When('the action is called with the amount, the address and the options', when.theActionIsCalledWithTheAmountTheAddressAndTheOptions, () => {
										...
										Then('it should create a transfer transaction using the address, the amount and the passphrase', then.itShouldCreateATransferTransactionUsingTheAddressTheAmountAndThePassphrase);
									...
									Given('an options object with second passphrase set to "secondPassphraseSource"', given.anOptionsObjectWithSecondPassphraseSetTo, () => {
										...
										Given('the passphrase and second passphrase can be retrieved from their sources', given.thePassphraseAndSecondPassphraseCanBeRetrievedFromTheirSources, () => {
											When('the action is called with the amount, the address and the options', when.theActionIsCalledWithTheAmountTheAddressAndTheOptions, () => {
												...
												Then('it should create a transfer transaction using the address, the amount, the passphrase and the second passphrase', then.itShouldCreateATransferTransactionUsingTheAddressTheAmountThePassphraseAndTheSecondPassphrase);
												Then('it should resolve to the created transaction', then.itShouldResolveToTheCreatedTransaction);
											});
										});
									});
