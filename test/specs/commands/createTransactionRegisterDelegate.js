...
describe('create transaction register delegate command', () => {
	...
	Given('a Vorpal instance with a UI and an active command that can prompt', given.aVorpalInstanceWithAUIAndAnActiveCommandThatCanPrompt, () => {
		Given('an action "create transaction register delegate"', given.anAction, () => {
						...
						Given('an empty options object', given.anEmptyOptionsObject, () => {
							...
							Given('the passphrase can be retrieved from its source', given.thePassphraseCanBeRetrievedFromItsSource, () => {
								When('the action is called with the delegate username and the options', when.theActionIsCalledWithTheDelegateUsernameAndTheOptions, () => {
									...
									Then('it should create a register delegate transaction using the passphrase and the delegate username', then.itShouldCreateARegisterDelegateTransactionUsingThePassphraseAndTheDelegateUsername);
							...
							Given('an options object with second passphrase set to "secondPassphraseSource"', given.anOptionsObjectWithSecondPassphraseSetTo, () => {
								...
								Given('the passphrase and second passphrase can be retrieved from their sources', given.thePassphraseAndSecondPassphraseCanBeRetrievedFromTheirSources, () => {
									When('the action is called with the delegate username and the options', when.theActionIsCalledWithTheDelegateUsernameAndTheOptions, () => {
										...
										Then('it should create a register delegate transaction using the passphrase, the second passphrase and the delegate username', then.itShouldCreateARegisterDelegateTransactionUsingThePassphraseTheSecondPassphraseAndTheDelegateUsername);
										Then('it should resolve to the created transaction', then.itShouldResolveToTheCreatedTransaction);
									});
								});
							});
