		...
		Given('a crypto instance has been initialised', given.aCryptoInstanceHasBeenInitialised, () => {
			Given('an action "encrypt message"', given.anAction, () => {
				Given('a passphrase "minute omit local rare sword knee banner pair rib museum shadow juice"', given.aPassphrase, () => {
					Given('a recipient "31919b459d28b1c611afb4db3de95c5769f4891c3f771c7dbcb53a45c452cc25"', given.aRecipient, () => {
						Given('a message "Some secret message\nthat spans multiple\n lines"', given.aMessage, () => {
							Given('an empty options object', given.anEmptyOptionsObject, () => {
								When('the action is called with the recipient and the options', when.theActionIsCalledWithTheRecipientAndTheOptions, () => {
								...
								Given('an error "Unknown data source type." occurs retrieving the inputs from their sources', given.anErrorOccursRetrievingTheInputsFromTheirSources, () => {
									When('the action is called with the recipient, the message and the options', when.theActionIsCalledWithTheRecipientTheMessageAndTheOptions, () => {
								...
								Given('the passphrase and message can be retrieved from their sources', given.thePassphraseAndMessageCanBeRetrievedFromTheirSources, () => {
									When('the action is called with the recipient, the message and the options', when.theActionIsCalledWithTheRecipientTheMessageAndTheOptions, () => {
										...
										Then('it should encrypt the message with the passphrase for the recipient', then.itShouldEncryptTheMessageWithThePassphraseForTheRecipient);
										Then('it should resolve to the result of encrypting the message', then.itShouldResolveToTheResultOfEncryptingTheMessage);
									});
								});
							});
							...
							Given('an options object with message set to "messageSource"', given.anOptionsObjectWithMessageSetTo, () => {
								...
								Given('the passphrase and message can be retrieved from their sources', given.thePassphraseAndMessageCanBeRetrievedFromTheirSources, () => {
									When('the action is called with the recipient and the options', when.theActionIsCalledWithTheRecipientAndTheOptions, () => {
										Then('it should get the inputs from sources using the Vorpal instance', then.itShouldGetTheInputsFromSourcesUsingTheVorpalInstance);
										Then('it should get the inputs from sources using the passphrase source with a repeating prompt', then.itShouldGetTheInputsFromSourcesUsingThePassphraseSourceWithARepeatingPrompt);
										Then('it should get the inputs from sources using the message source', then.itShouldGetTheInputsFromSourcesUsingTheMessageSource);
										Then('it should encrypt the message with the passphrase for the recipient', then.itShouldEncryptTheMessageWithThePassphraseForTheRecipient);
										Then('it should resolve to the result of encrypting the message', then.itShouldResolveToTheResultOfEncryptingTheMessage);
									});
								});
