...
describe('download utils', () => {
  ...
  describe('#download', () => {
    ...
    it.skip('should get latest version', async () => {
      ...
      await downloadUtil.download('http://download/lisk/version.txt', 'file/path');
  ...
  describe('#validateChecksum', () => {
    it('should throw error when failed to validate checksum', async () => {
      ...
      return expect(downloadUtil.validateChecksum('file/path', 'test.gz')).to.rejectedWith('Checksum validation failed error with error: invalid checksum');
    });

    it('should successfully validate checksum', async () => {
      execStub.resolves({ stdout: 'OK' });
      return expect(downloadUtil.validateChecksum('file/path', 'test.gz')).to.not.throw;
    });
  });
  ...
  describe('#extract', () => {
    it('should throw error when failed to extract', async () => {
      ...
      return expect(downloadUtil.extract('file/path', 'test.gz', 'output/dir')).to.rejectedWith('Extraction failed with error: invalid filepath');
    });

    it('should successfully extract file', async () => {
      execStub.resolves({ stdout: 'OK' });

      const result = await downloadUtil.extract('file/path', 'test.gz', 'output/dir');
  ...
  describe('#downloadLiskAndValidate', () => {
    ...
    it('should be able to download and validate release', async () => {
      await downloadUtil.downloadLiskAndValidate('output/dir', 'release/url', '2.0.0');
