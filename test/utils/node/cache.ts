...
describe('cache node utils', () => {
  describe('#isCacheRunning', () => {
    ...
    describe('when installation exists', () => {
      beforeEach(() => {
        sandbox.stub(workerProcess, 'exec').resolves({ stdout: 'PONG', stderr: null });
      });
  ...
  describe('#startCache', () => {
    describe('when installation does not exists', () => {
      it('should throw error', async () => {
        try {
          ...
          return expect(error.message.search('No such file or directory') >= 0).to.be.true;
        }
      });
    });
    ...
    describe('when installation exists', () => {
      ...
      it('should throw error when failed to stop', async () => {
        workerProcessStub.resolves({ stdout: 'Failed to start redis', stderr: 'Failed to start redis' });

        try {
          return await startCache('/tmp/dummypath', NETWORK.MAINNET);
        } catch (error) {
          console.log(error);
          return expect(error.message.search('Failed to start Redis-Server.') >= 0).to.be.true;
        }
      });
    });
  });
  ...
  describe('#stopCache', () => {
    describe('when installation does not exists', () => {
      it('should throw error', async () => {
        try {
          ...
          return expect(error.message.search('Config file not exists in path') >= 0).to.be.true;
        }
      });
    });
    ...
    describe('when installation exists', () => {
      ...
      it('should throw error when failed to stop', async () => {
        configStub.returns({ password: 'lisk' });
        workerProcessStub.resolves({ stdout: 'Failed to stop redis', stderr: 'Failed to stop redis' });

        try {
          return await stopCache('/tmp/dummypath', NETWORK.MAINNET);
        } catch (error) {
          console.log(error);
          return expect(error.message.search('Failed to stop Redis-Server.') >= 0).to.be.true;
        }
      });
    });
  });
