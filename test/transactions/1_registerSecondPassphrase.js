...
describe('#registerSecondPassphrase transaction', () => {
	...
	describe('returned register second passphrase transaction', () => {
		...
		it('should have an id string', () => {
			(registerSecondPassphraseTransaction).should.have.property('id').and.be.type('string');
		});

		it('should have type number equal to 1', () => {
			(registerSecondPassphraseTransaction).should.have.property('type').and.be.type('number').and.equal(1);
		});
		...
		it('should have recipientId equal to null', () => {
			(registerSecondPassphraseTransaction).should.have.property('recipientId').and.be.null();
		});

		it('should have senderPublicKey hex string equal to sender public key', () => {
			(registerSecondPassphraseTransaction).should.have.property('senderPublicKey').and.be.hexString().and.equal(publicKey);
		});

		it('should have timestamp number equal to result of time.getTimeWithOffset', () => {
			(registerSecondPassphraseTransaction).should.have.property('timestamp').and.be.type('number').and.equal(timeWithOffset);
		});

		it('should have signature hex string', () => {
			(registerSecondPassphraseTransaction).should.have.property('signature').and.be.hexString();
		});

		it('should be signed correctly', () => {
			const result = cryptoModule.verifyTransaction(registerSecondPassphraseTransaction);
			(result).should.be.ok();
		});
		...
		it('should have asset object', () => {
			(registerSecondPassphraseTransaction).should.have.property('asset').and.not.be.empty();
		});

		it('should not have a signSignature property', () => {
			(registerSecondPassphraseTransaction).should.not.have.property('signSignature');
		});
