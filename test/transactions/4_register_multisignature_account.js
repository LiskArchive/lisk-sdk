...
import registerMultisignatureAccount, {
	signMultisignature,
} from 'transactions/4_register_multisignature_account';
// Require is used for stubbing
const time = require('transactions/utils/time');

describe('4_register_multisignature_account module', () => {
			...
			describe('returned register multisignature transaction', () => {
				...
				describe('multisignature asset', () => {
					...
					it('should have a min number equal to provided minimum', () => {
						return expect(
							registerMultisignatureTransaction.asset.multisignature,
						)
	...
	describe('#signMultisignature', () => {
		describe('when invalid transaction is used', () => {
			...
			it('should throw an Error', () => {
				return expect(
					signMultisignature.bind(null, transaction, passphrase),
				).to.throw();
			});
		});
		...
		describe('when valid transaction and passphrase is used', () => {
			...
			let signature;
			beforeEach(() => {
				signature = signMultisignature(transaction, account.passphrase);
				return Promise.resolve(true);
			});

			it('should have the same transaction id as the input', () => {
				return expect(signature.transactionId).to.equal(transaction.id);
			});

			it('should have the corresponding public key with the passphrase', () => {
				return expect(signature.publicKey).to.equal(account.publicKey);
			});

			it('should have all the keys for signature type', () => {
				return expect(signature).to.have.all.keys([
					'transactionId',
					'publicKey',
					'signature',
				]);
