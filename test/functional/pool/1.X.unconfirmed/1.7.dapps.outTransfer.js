...
describe('POST /api/transactions (unconfirmed type 7 on top of type 1)', function () {
	...
	describe('outTransfer', function () {
		...
		it('using second signature with an account that has a pending second passphrase registration should fail', function () {
			transaction = node.lisk.transfer.createOutTransfer(randomUtil.guestbookDapp.transactionId, randomUtil.transaction().id, randomUtil.account().address, 10 * node.normalizer, account.password, account.secondPassword);
		...
		it('using no second signature with an account that has a pending second passphrase registration should be ok', function () {
			transaction = node.lisk.transfer.createOutTransfer(randomUtil.guestbookDapp.transactionId, randomUtil.transaction().id, randomUtil.account().address, 10 * node.normalizer, account.password);
