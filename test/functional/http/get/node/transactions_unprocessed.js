	...
	describe('/transactions', () => {
		describe('/unprocessed @unstable', () => {
			...
			before(() => {
				...
				return Promise.map(transactionList, transaction => { return sendTransactionPromise(transaction); }).then(responses => {
					responses.map(res => {
...
return UnProcessedEndpoint.makeRequest({
 ...
 return UnProcessedEndpoint.makeRequest({ id: '79fjdfd' }, 400).then(res => {
...
return UnProcessedEndpoint.makeRequest({ id: '1111111111111111' }, 200).then(res => {
...
return UnProcessedEndpoint.makeRequest({ senderId: '79fjdfd' }, 400).then(res => {
						expectSwaggerParamError(res, 'senderId');
					});
});

				it('using valid senderId should be ok', () => {
return UnProcessedEndpoint.makeRequest({ senderId: accountFixtures.genesis.address }, 200).then(res => {
						expect(res.body.data).to.not.empty;
						expect(res.body.data.length).to.be.at.least(numOfTransactions);
						res.body.data.map(function (transaction) {
 ...
 return UnProcessedEndpoint.makeRequest({ senderId: '1631373961111634666L' }, 200).then(res => {
						expect(res.body.data).to.be.empty;
					});
});
			});
 ...
 return UnProcessedEndpoint.makeRequest({ senderPublicKey: '79fjdfd' }, 400).then(res => {
						expectSwaggerParamError(res, 'senderPublicKey');
					});
});

				it('using valid senderPublicKey should be ok', () => {
return UnProcessedEndpoint.makeRequest({ senderPublicKey: accountFixtures.genesis.publicKey }, 200).then(res => {
						expect(res.body.data).to.not.empty;
						expect(res.body.data.length).to.be.at.least(numOfTransactions);
						res.body.data.map(function (transaction) {
 ...
 return UnProcessedEndpoint.makeRequest({ senderPublicKey: 'c094ebee7ec0c50ebeeaaaa8655e089f6e1a604b83bcaa760293c61e0f18ab6f' }, 200).then(res => {
						expect(res.body.data).to.be.empty;
					});
});
			});
 ...
 return UnProcessedEndpoint.makeRequest({ recipientId: '79fjdfd' }, 400).then(res => {
						expectSwaggerParamError(res, 'recipientId');
					});
});

				it('using valid recipientId should be ok', () => {
return UnProcessedEndpoint.makeRequest({ recipientId: account.address }, 200).then(res => {
						expect(res.body.data).to.not.empty;
						expect(res.body.data.length).to.be.at.least(numOfTransactions);
						res.body.data.map(function (transaction) {
 ...
 return UnProcessedEndpoint.makeRequest({ recipientId: '1631373961111634666L' }, 200).then(res => {
						expect(res.body.data).to.be.empty;
					});
});
			});
 ...
 return UnProcessedEndpoint.makeRequest({ recipientPublicKey: '79fjdfd' }, 400).then(res => {
						expectSwaggerParamError(res, 'recipientPublicKey');
					});
});

				it('using valid recipientPublicKey should be ok', () => {
return UnProcessedEndpoint.makeRequest({ recipientPublicKey: account.publicKey }, 200).then(res => {
						expect(res.body.data).to.not.empty;
						expect(res.body.data.length).to.be.at.least(numOfTransactions);
						res.body.data.map(function (transaction) {
 ...
 return UnProcessedEndpoint.makeRequest({ recipientPublicKey: 'c094ebee7ec0c50ebeeaaaa8655e089f6e1a604b83bcaa760293c61e0f18ab6f' }, 200).then(res => {
						expect(res.body.data).to.be.empty;
					});
});
			});
 ...
 return UnProcessedEndpoint.makeRequest({ limit: -1 }, 400).then(res => {
						expectSwaggerParamError(res, 'limit');
					});
});

				it('using limit > 100 should fail', () => {
return UnProcessedEndpoint.makeRequest({ limit: 101 }, 400).then(res => {
						expectSwaggerParamError(res, 'limit');
					});
});

				it('using limit = 2 should return 2 transactions', () => {
 return UnProcessedEndpoint.makeRequest({ limit: 2 }, 200).then(res => {
 ...
 return UnProcessedEndpoint.makeRequest({ offset: 'one' }, 400).then(res => {
						expectSwaggerParamError(res, 'offset');
					});
});
 ...
 return UnProcessedEndpoint.makeRequest({ sort: 'amount:asc' }, 200).then(res => {
							expect(res.body.data).to.not.be.empty;
 ...
 return UnProcessedEndpoint.makeRequest({ sort: 'amount:desc' }, 200).then(res => {
							expect(res.body.data).to.not.be.empty;
...
return UnProcessedEndpoint.makeRequest({ sort: 'fee:asc' }, 200).then(res => {
							expect(res.body.data).to.not.be.empty;
...
return UnProcessedEndpoint.makeRequest({ sort: 'fee:desc' }, 200).then(res => {
							expect(res.body.data).to.not.be.empty;
				...
				describe('type', () => {
					it('sorted by fee:asc should be ok', () => {
return UnProcessedEndpoint.makeRequest({ sort: 'type:asc' }, 200).then(res => {
							expect(res.body.data).to.not.be.empty;
					...
					it('sorted by fee:desc should be ok', () => {
 return UnProcessedEndpoint.makeRequest({ sort: 'type:desc' }, 200).then(res => {
							expect(res.body.data).to.not.be.empty;
 ...
 return UnProcessedEndpoint.makeRequest({ sort: 'timestamp:asc' }, 200).then(res => {
							expect(res.body.data).to.not.be.empty;
 ...
 return UnProcessedEndpoint.makeRequest({ sort: 'timestamp:desc' }, 200).then(res => {
							expect(res.body.data).to.not.be.empty;
 ...
 return UnProcessedEndpoint.makeRequest({ sort: 'id:asc' }, 400).then(res => {
						expectSwaggerParamError(res, 'sort');
					});
});
			});
