...
describe('GET /accounts', () => {
	...
	describe('?', () => {
		describe('address', () => {
			it('using known address should be ok', () => { return accountsEndpoint.makeRequest({ address: accountFixtures.genesis.address }, 200); });

			it('using known address and empty publicKey should return empty result', () => {
 return accountsEndpoint.makeRequest({ address: accountFixtures.genesis.address, publicKey: '' }, 200).then(res => {
					expect(res.body.data).to.have.length(0);
				});
});

			it('using known lowercase address should be ok', () => { return accountsEndpoint.makeRequest({ address: accountFixtures.genesis.address.toLowerCase() }, 200); });

			it('using unknown address should return empty result', () => {
return accountsEndpoint.makeRequest({ address: account.address }, 200).then(res => {
					expect(res.body.data).to.have.length(0);
				});
});

			it('using invalid address should fail', () => {
 return accountsEndpoint.makeRequest({ address: 'InvalidAddress' }, 400).then(res => {
...
return accountsEndpoint.makeRequest({ address: '' }, 400).then(res => {
		...
		describe('publicKey', () => {
			it('using known publicKey should be ok', () => { return accountsEndpoint.makeRequest({ publicKey: accountFixtures.genesis.publicKey }, 200); });

			it('using known publicKey and empty address should fail', () => {
 return accountsEndpoint.makeRequest({ publicKey: accountFixtures.genesis.publicKey, address: '' }, 400).then(res => {
					expectSwaggerParamError(res, 'address');
				});
});

			it('using unknown publicKey should return empty result', () => {
 return accountsEndpoint.makeRequest({ publicKey: account.publicKey }, 200).then(res => {
					expect(res.body.data).to.have.length(0);
				});
});

			it('using invalid publicKey should fail', () => {
return accountsEndpoint.makeRequest({ publicKey: 'invalidPublicKey' }, 400).then(res => {
					expectSwaggerParamError(res, 'publicKey');
				});
});

			it('using invalid publicKey (integer) should fail', () => {
 return accountsEndpoint.makeRequest({ publicKey: '123' }, 400).then(res => {
					expectSwaggerParamError(res, 'publicKey');
				});
});

			it('using empty publicKey should return empty results', () => {
 return accountsEndpoint.makeRequest({ publicKey: '' }, 200).then(res => {
					expect(res.body.data).to.have.length(0);
				});
});

			it('using empty publicKey and address should fail', () => {
 return accountsEndpoint.makeRequest({ publicKey: '', address: '' }, 400).then(res => {
					expectSwaggerParamError(res, 'address');
				});
});

			it('using known address and matching publicKey should be ok', () => {
return accountsEndpoint.makeRequest({ publicKey: accountFixtures.genesis.publicKey, address: accountFixtures.genesis.address }, 200).then(res => {
...
return accountsEndpoint.makeRequest({ publicKey: account.publicKey, address: accountFixtures.genesis.address }, 200).then(res => {
					expect(res.body.data).to.have.length(0);
				});
});
		});
...
return apiHelpers.sendTransactionPromise(creditTransaction).then(res => {
				...
				}).then(() => { return apiHelpers.sendTransactionPromise(signatureTransaction); }).then(res => {
					expect(res.statusCode).to.be.eql(200);
 ...
 return accountsEndpoint.makeRequest({ secondPublicKey: secondPublicKeyAccount.secondPublicKey }, 200).then(res => {
 ...
 return accountsEndpoint.makeRequest({ secondPublicKey: account.secondPublicKey }, 200).then(res => {
					expect(res.body.data).to.have.length(0);
				});
});

			it('using invalid secondPublicKey should fail', () => {
return accountsEndpoint.makeRequest({ secondPublicKey: 'invalidPublicKey' }, 400).then(res => {
...
return accountsEndpoint.makeRequest({ username: '' }, 400).then(res => {
 ...
 return accountsEndpoint.makeRequest({ username: _.repeat('a', 21) }, 400).then(res => {
					expectSwaggerParamError(res, 'username');
				});
});

			it('using valid username name should result account', () => {
 return accountsEndpoint.makeRequest({ username: accountFixtures.existingDelegate.delegateName }, 200).then(res => {
					expect(res.body.data).to.have.length(1);
...
return accountsEndpoint.makeRequest({ limit: 102 }, 400).then(res => {
 ...
 return accountsEndpoint.makeRequest({ limit: 5 }, 200).then(res => {
		...
		describe('sort @unstable', () => {
			it('using sort = invalid should return error', () => { return accountsEndpoint.makeRequest({ sort: 'invalid' }, 400); });

			it('using no sort return accounts sorted by balance in asending order as default behavior', () => {
return accountsEndpoint.makeRequest({ sort: 'balance:asc' }, 200).then(res => {
...
return accountsEndpoint.makeRequest({ sort: 'balance:desc' }, 200).then(res => {
					var balances = _(res.body.data).map('balance').value();
 ...
 return accountsEndpoint.makeRequest({ sort: 'balance:asc', offset: 1, limit: 5 }, 200).then(res => {
					var balances = _(res.body.data).map('balance').value();
 ...
 return accountsEndpoint.makeRequest({ address: accountFixtures.existingDelegate.address }, 200).then(res => {
				expect(res.body.data[0].address).to.be.eql(accountFixtures.existingDelegate.address);
				expect(res.body.data[0].publicKey).to.be.eql(accountFixtures.existingDelegate.publicKey);
 ...
 return accountsEndpoint.makeRequest({ address: accountFixtures.genesis.address }, 200).then(res => {
				expect(res.body.data[0].address).to.be.eql(accountFixtures.genesis.address);
				expect(res.body.data[0].publicKey).to.be.eql(accountFixtures.genesis.publicKey);
