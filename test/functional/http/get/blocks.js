...
describe('GET /blocks', () => {
	...
	describe('?', () => {
		describe('blockId', () => {
			it('using invalid blockId = "InvalidId" format should fail with error', () => blocksEndpoint.makeRequest({ blockId: 'InvalidId' }, 400).then(res => {
			...
			it('using unknown id should return empty blocks array', () => blocksEndpoint.makeRequest({ blockId: '9928719876370886655' }, 200).then(res => {
		...
		describe('height', () => {
			it('using invalid height = 0 should fail with error', () => blocksEndpoint.makeRequest({ height: 0 }, 400).then(res => {
					expectSwaggerParamError(res, 'height');
				}));

			it('using invalid height = -1 should fail with error', () => blocksEndpoint.makeRequest({ height: 0 }, 400).then(res => {
					expectSwaggerParamError(res, 'height');
				}));

			it('using correct params should be ok', () => blocksEndpoint.makeRequest({ height: block.blockHeight }, 200).then(res => {
		...
		describe('generatorPublicKey', () => {
			it('using invalid generatorPublicKey = "InvalidKey" format should fail with error', () => blocksEndpoint.makeRequest({ generatorPublicKey: 'InvalidKey' }, 400).then(res => {
					expectSwaggerParamError(res, 'generatorPublicKey');
				}));

			it('using correct params should be ok', () => blocksEndpoint.makeRequest({ generatorPublicKey: block.generatorPublicKey }, 200).then(res => {
		...
		describe('sort', () => {
			describe('height', () => {
				it('using "height:asc" should be ok', () => blocksEndpoint.makeRequest({ sort: 'height:asc' }, 200).then(res => {
						expectHeightCheck(res);
						expect(_(res.body.data).map('height').sortNumbers()).to.be.eql(_.map(res.body.data, 'height'));
					}));

				it('using "height:desc" should be ok', () => blocksEndpoint.makeRequest({ sort: 'height:desc' }, 200).then(res => {
						expectHeightCheck(res);
						expect(_(res.body.data).map('height').sortNumbers('desc')).to.be.eql(_.map(res.body.data, 'height'));
					}));

				it('using empty params should sort results by descending height', () => blocksEndpoint.makeRequest({}, 200).then(res => {
						expectHeightCheck(res);
						expect(_(res.body.data).map('height').sortNumbers('desc')).to.be.eql(_.map(res.body.data, 'height'));
					}));
			});
			...
			describe('totalAmount', () => {
				it('using "totalAmount:asc" should be ok', () => blocksEndpoint.makeRequest({ sort: 'totalAmount:asc' }, 200).then(res => {
						expectHeightCheck(res);
				...
				it('using "totalAmount:desc" should be ok', () => blocksEndpoint.makeRequest({ sort: 'totalAmount:desc' }, 200).then(res => {
						expectHeightCheck(res);
			...
			describe('totalFee', () => {
				it('using "totalFee:asc" should be ok', () => blocksEndpoint.makeRequest({ sort: 'totalFee:asc' }, 200).then(res => {
						expectHeightCheck(res);
				...
				it('using "totalFee:desc" should be ok', () => blocksEndpoint.makeRequest({ sort: 'totalFee:desc' }, 200).then(res => {
						expectHeightCheck(res);
			...
			describe('timestamp', () => {
				it('using "timestamp:asc" should be ok', () => blocksEndpoint.makeRequest({ sort: 'timestamp:asc' }, 200).then(res => {
						expectHeightCheck(res);
						expect(_(res.body.data).map('timestamp').sortNumbers()).to.be.eql(_.map(res.body.data, 'timestamp'));
					}));

				it('using "timestamp:desc" should be ok', () => blocksEndpoint.makeRequest({ sort: 'timestamp:desc' }, 200).then(res => {
						expectHeightCheck(res);
		...
		describe('limit', () => {
			it('using string should return bad request response', () => blocksEndpoint.makeRequest({ limit: 'one' }, 400).then(res => {
					expectSwaggerParamError(res, 'limit');
				}));

			it('using -1 should return bad request response', () => blocksEndpoint.makeRequest({ limit: -1 }, 400).then(res => {
					expectSwaggerParamError(res, 'limit');
				}));

			it('using 0 should return bad request response', () => blocksEndpoint.makeRequest({ limit: 0 }, 400).then(res => {
					expectSwaggerParamError(res, 'limit');
				}));

			it('using 1 should be ok', () => blocksEndpoint.makeRequest({ limit: 1 }, 200).then(res => {
					expect(res.body.data).to.have.length(1);
				}));

			it('using 100 should be ok', () => blocksEndpoint.makeRequest({ limit: 100 }, 200).then(res => {
					expect(res.body.data.length).to.be.at.most(100);
				}));

			it('using > 100 should return bad request response', () => blocksEndpoint.makeRequest({ limit: 101 }, 400).then(res => {
					expectSwaggerParamError(res, 'limit');
				}));
		});
