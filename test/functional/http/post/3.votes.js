...
describe('POST /api/transactions (type 3) votes', function () {
	...
	before(function () {
		// Crediting all the accounts and registering delegates for the two scenarios
		var promises = [];
		...
		promises.push(creditAccountPromise(accountDuplicate.address, constants.fees.vote * 4));
	...
	describe('transactions processing', function () {
		...
		it('voting with invalid vote format should fail', function () {
			transaction = node.lisk.vote.createVote(delegateAccount.password, ['++' + node.eAccount.publicKey.slice(0, -1)]);
		...
		it('upvoting ' + (constants.activeDelegates + 1) + ' delegates (number of actived delegates + 1) separately should be ok but just 101 votes confirmed', function () {
			...
			return sendTransactionPromise(transaction).then(function (res) {
				...
				transaction = node.lisk.vote.createVote(accountMaxVotesPerAccount.password, delegatesMaxVotesPerAccount.slice(0, 33).map(function (delegate) {
					return '+' + delegate.publicKey;
				}));
				...
				return sendTransactionPromise(transaction).then(function (res) {
					...
					transaction = node.lisk.vote.createVote(accountMaxVotesPerAccount.password, delegatesMaxVotesPerAccount.slice(33, 66).map(function (delegate) {
						return '+' + delegate.publicKey;
					}));
					...
					return sendTransactionPromise(transaction).then(function (res) {
						...
						transaction = node.lisk.vote.createVote(accountMaxVotesPerAccount.password, delegatesMaxVotesPerAccount.slice(66, 99).map(function (delegate) {
							return '+' + delegate.publicKey;
						}));
						...
						return sendTransactionPromise(transaction).then(function (res) {
							...
							transaction = node.lisk.vote.createVote(accountMaxVotesPerAccount.password, delegatesMaxVotesPerAccount.slice(99, 102).map(function (delegate) {
								return '+' + delegate.publicKey;
							}));
	...
	describe('unconfirmed state after validation', function () {
		...
		it('downvoting with valid params and duplicate submission should be ok but just first transaction to arrive will be confirmed', function () {
			transaction = node.lisk.vote.createVote(accountDuplicate.password, ['-' + node.eAccount.publicKey]);
