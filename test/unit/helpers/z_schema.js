...
const { ADDITIONAL_DATA } = require('../../../helpers/constants.js');
...
describe('schema - custom formats', () => {
	...
	describe('additionalData', () => {
		...
		it('should return false if string is longer than maxLength (either chars or bytes)', function() {
			var invalidData = [];
			invalidData.push(
				`${randomstring.generate(ADDITIONAL_DATA.maxLength - 1)}现`
			);
			invalidData.push(randomstring.generate(ADDITIONAL_DATA.maxLength + 1));
		...
		it('should return true if string is between minLength and maxLength', function() {
			var validData = [];
			validData.push(randomstring.generate(ADDITIONAL_DATA.minLength));
			validData.push(randomstring.generate(ADDITIONAL_DATA.maxLength));
