...
describe('ed', function () {
	...
	describe.skip('verifyString', function () {
		...
		it('should return true when proper hex string signature is checked with matching string hex public key', function () {
			var verified = ed.verifyString(Buffer.from(JSON.stringify(messageToSign)), signature.toString('hex'), keys.publicKey.toString('hex'));
			expect(verified).to.be.ok;
		});

		it('should return true when proper Buffer signature is checked with matching string hex public key', function () {
			var verified = ed.verifyString(Buffer.from(JSON.stringify(messageToSign)), signature, keys.publicKey.toString('hex'));
			expect(verified).to.be.ok;
		});

		it('should return true when proper hex string signature is checked with matching Buffer public key', function () {
			var verified = ed.verifyString(Buffer.from(JSON.stringify(messageToSign)), signature.toString('hex'), keys.publicKey);
			expect(verified).to.be.ok;
		});

		it('should return false when malformed hex string signature is checked with matching Buffer public key', function () {
			var verified = ed.verifyString(Buffer.from(JSON.stringify(messageToSign)), 'wrongSignature'.toString('hex'), keys.publicKey);
			expect(verified).not.to.be.ok;
		});

		it('should return false proper signature and proper publicKey is check against malformed data', function () {
			var verified = ed.verifyString(Buffer.from('malformed data'), signature.toString('hex'), keys.publicKey);
			expect(verified).not.to.be.ok;
		});

		it('should throw an error when proper non hex string signature is checked with matching string hex public key', function () {
			// var verified = ed.verify(Buffer.from(JSON.stringify(messageToSign)), signature.toString(), keys.publicKey.toString('hex'));
			expect(ed.verifyString.bind(null, Buffer.from(JSON.stringify(messageToSign)), signature.toString(), keys.publicKey.toString('hex'))).to.throw();
		});

		it('should throw an error when proper non hex string signature is checked with matching string non hex public key', function () {
			expect(ed.verifyString.bind(null, Buffer.from(JSON.stringify(messageToSign)), signature.toString('hex'), keys.publicKey.toString())).to.throw();
		});
