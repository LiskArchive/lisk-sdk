...
describe('node', () => {
	...
	before(done => {
		application.init({ sandbox: { name: 'lisk_test_modules_node' } }, (err, scope) => {
			library = scope;
	...
	describe('internal', () => {
		...
		function updateForgingStatus(testDelegate, action, cb) {
			node_module.internal.getForgingStatus(testDelegate.publicKey, (err, res) => {
		...
		describe('toggleForgingStatus', () => {
			...
			it('should return error with invalid key', done => {
				node_module.internal.toggleForgingStatus(testDelegate.publicKey, 'Invalid key', err => {
			...
			it('should return error with invalid publicKey', done => {
				...
				node_module.internal.toggleForgingStatus(invalidPublicKey, defaultKey, err => {
			...
			it('should return error with non delegate account', done => {
				node_module.internal.toggleForgingStatus(accountFixtures.genesis.publicKey, accountFixtures.genesis.password, err => {
			...
			it('should toggle forging from enabled to disabled', done => {
				updateForgingStatus(testDelegate, 'enable', err => {
					...
					node_module.internal.toggleForgingStatus(testDelegate.publicKey, defaultKey, (err, res) => {
						expect(err).to.not.exist;
						expect(res).to.eql({
							publicKey: testDelegate.publicKey,
							forging: false
