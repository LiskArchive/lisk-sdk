...
describe('transport', function () {
	...
	function insertValidPeerSignedWithOwnerKey (peer, cb) {
		...
		// var verified = ed.verify(Buffer.from(JSON.stringify(peer.object())), Buffer.from(signature, 'hex'), Buffer.from(constants.headers.publicKey, 'hex'));



		transport.internal.acceptPeer({peer: peer.object(), signature: validPeerValidSignature}, function (err, result) {
	...
	describe('acceptPeer', function () {
		...
		it('should not call peers.update without parameters', function (done) {
			transport.internal.acceptPeer(null, function (err) {
				expect(err).equal('Wrong accept peer query format.');
		...
		describe('insert peer', function () {
			...
			it('should accept new peer only when peer\'s data is signed with own private key', function (done) {
				...
				transport.internal.acceptPeer({peer: validPeer.object(), signature: validPeerValidSignature}, function (err, result) {
			...
			it('should not accept new peer when peer\'s data is signed with different than owner\'s private key', function (done) {
				...
				transport.internal.acceptPeer({peer: validPeer.object(), signature: validPeerDifferentSignature}, function (err, result) {
			...
			it('should not accept new peer when peer\'s data is malformed but signed with owner\'s private key', function (done) {
				...
				transport.internal.acceptPeer({peer: malformedPeer.object(), signature: validPeerValidSignature}, function (err, result) {
					expect(err).equal('Unsuccessful validation of update peer signature');
					expect(modules.peers.update.calledOnce).not.to.be.ok;
					done();
				});
		...
		describe('update peer', function () {
			...
			it('should not update different peer\'s data which is not on a list signed by peer\'s private key if peer is on a peers list', function (done) {
				...
				insertValidPeerSignedWithOwnerKey(validPeer, function () {
					...
					transport.internal.acceptPeer({peer: nextDifferentPeer.object(), signature: nextDifferentPeerSignature}, function (err, result) {
						expect(err).equal('Unsuccessful validation of update peer signature');
						expect(modules.peers.update.calledOnce).not.to.be.ok;
						done();
					});
	...
	describe('removePeer', function () {
		...
		describe('removing by itself', function () {
			...
			it('should remove peer only when peer\'s data is signed with own private key', function (done) {
				...
				transport.internal.removePeer({peer: validPeer.object(), signature: validPeerValidSignature}, function (err, result) {
			...
			it('should not remove peer when peer\'s data is signed with different than owner\'s private key', function (done) {
				...
				transport.internal.removePeer({peer: validPeer.object(), signature: validPeerDifferentSignature}, function (err, result) {
					expect(err).equal('Unsuccessful validation of update peer signature');
			...
			it('should not remove peer when peer\'s data is malformed but signed with owner\'s private key', function (done) {
				...
				var malformedPeer = new Peer(_.clone(randomPeer));
				malformedPeer.height -= 1;
				transport.internal.removePeer({peer: malformedPeer.object(), signature: validPeerValidSignature}, function (err, result) {
					expect(err).equal('Unsuccessful validation of update peer signature');
					expect(modules.peers.remove.calledOnce).not.to.be.ok;
					done();
				});
		...
		describe('removing by other peer', function () {
			...
			it('should not remove valid peer\'s signed by peer\'s private key if it is not on a peers list', function (done) {
			...
			it('should remove valid peer\'s signed by peer\'s private key if it is on a peers list', function (done) {
			...
			it('should not remove different peer\'s data which is not on a list signed by peer\'s private key if peer is on a peers list', function (done) {
				...
				insertValidPeerSignedWithOwnerKey(validPeer, function () {
					...
					transport.internal.removePeer({peer: nextDifferentPeer.object(), signature: nextDifferentPeerSignature}, function (err, result) {
						expect(err).equal('Unsuccessful validation of update peer signature');
						expect(modules.peers.remove.calledOnce).not.to.be.ok;
						done();
					});
			...
			it('should not remove different peer\'s data which is on a list signed by peer\'s private key if peer is on a peers list', function (done) {
