...
function calcBlockReward(height, reward, done) {
	return db
		.query(sql.calcBlockReward, { height: height })
		.then(function(rows) {
...
function calcSupply(height, supply, done) {
	return db
		.query(sql.calcSupply, { height: height })
		.then(function(rows) {
			expect(rows).to.be.an('array');
			expect(rows.length).to.equal(1);
			expect(rows[0]).to.be.an('object');
...
function calcBlockReward_test(height_start, height_end, expected_reward, done) {
	return db
		.query(sql.calcBlockReward_test, {
			height_start: height_start,
			height_end: height_end,
			expected_reward: expected_reward,
		})
...
describe('BlockRewardsSQL @slow', function() {
	before(function(done) {
		application.init(
			{ sandbox: { name: 'lisk_test_sql_block_rewards' } },
			function(err, scope) {
				db = scope.db;
		...
		it('SQL rewards should be equal to those in constants', function(done) {
			db
				...
				.then(function(rows) {
					...
					// Checking milestones
					expect(Number(rows[0].milestones[0])).to.equal(
						constants.rewards.milestones[0]
					);
					expect(Number(rows[0].milestones[1])).to.equal(
						constants.rewards.milestones[1]
					);
					expect(Number(rows[0].milestones[2])).to.equal(
						constants.rewards.milestones[2]
					);
					expect(Number(rows[0].milestones[3])).to.equal(
						constants.rewards.milestones[3]
					);
					expect(Number(rows[0].milestones[4])).to.equal(
	...
	describe('checking SQL function calcBlockReward(int)', function() {
		...
		it('when height == (milestoneFour * 1000) should overflow int and return error', function(done) {
			db
				.query(sql.calcBlockReward, { height: 13451520 * 1000 })
				.then(function(rows) {
				...
				.catch(function(err) {
					...
					expect(err.message).to.contain(
						'function calcblockreward(bigint) does not exist'
					);
					done();
				});
		});
	...
	describe('checking SQL function calcSupply(int)', function() {
		...
		it('when height == (milestoneFour * 1000) should overflow int and return error', function(done) {
			db
				.query(sql.calcSupply, { height: 13451520 * 1000 })
				.then(function(rows) {
					done('Should not pass');
				})
				.catch(function(err) {
					expect(err).to.be.an('error');
					expect(err.message).to.contain(
						'function calcsupply(bigint) does not exist'
					);
					done();
				});
		});
