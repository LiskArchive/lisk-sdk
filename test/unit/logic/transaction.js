...
describe('transaction', () => {
	...
	before(done => {
		...
		application.init({ sandbox: { name: 'lisk_test_logic_transactions' } }, (err, scope) => {
	...
	describe('apply', () => {
		...
		it('should throw an error with no param', () => {
			expect(() => { transactionLogic.apply(); }).to.throw();
		});
		...
		it('should subtract balance from sender account on valid transaction', done => {
			accountModule.getAccount({ publicKey: validTransaction.senderPublicKey }, (err, accountBefore) => {
				...
				transactionLogic.apply(validTransaction, dummyBlock, sender, () => {
					accountModule.getAccount({ publicKey: validTransaction.senderPublicKey }, (err, accountAfter) => {
						expect(err).to.not.exist;
	...
	describe('undo', () => {
		...
		it('should not update sender balance when transaction is invalid', done => {
			...
			accountModule.getAccount({ publicKey: transaction.senderPublicKey }, (err, accountBefore) => {
				var balanceBefore = new bignum(accountBefore.balance.toString());

				transactionLogic.undo(transaction, dummyBlock, sender, () => {
					accountModule.getAccount({ publicKey: transaction.senderPublicKey }, (err, accountAfter) => {
						var balanceAfter = new bignum(accountAfter.balance.toString());
	...
	describe('applyUnconfirmed', () => {
		...
		it('should throw an error with no param', () => {
			expect(() => { transactionLogic.applyUnconfirmed(); }).to.throw();
		});
