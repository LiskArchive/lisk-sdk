		...
		describe('methods', function () {
			...
			describe('upsert', function () {
				...
				it('should update only the columns specified in columnSet.update', function () {
					...
					return db.accounts.upsert(originalAccount, 'address').then(function () {
						return db.accounts.upsert(originalAccount, 'address', updatedAccount).then(function () {
				...
				it('should update data attributes specified as "data" if argument "updateData" is missing', function () {
					...
					return db.accounts.upsert(originalAccount, 'address').then(function () {
						return db.accounts.upsert(updatedAccount, 'address').then(function () {
							return db.accounts.list({ address: originalAccount.address }).then(function (result) {
								expect(result.length).to.eql(1);
				...
				it('should match the values for conflict keys case sensitively', function () {
					...
					return db.accounts.upsert(originalAccount, 'username').then(function () {
						return db.accounts.upsert(updatedAccount, 'username').then(function () {
				...
				it('should match the multiple conflict keys with AND composite', function () {
					...
					return db.accounts.upsert(originalAccount, 'address').then(function () {
						return db.accounts.upsert(updatedAccount, ['username', 'u_username']).then(function () {
							return db.accounts.list({ username: updatedAccount.username }).then(function (result) {
								expect(result.length).to.eql(1);
			...
			describe('update', function () {
				...
				it('should update all columns specified in db.accounts.cs.update', function () {
					...
					return db.accounts.insert(account).then(function () {
						return db.accounts.update(account.address, updateAccount).then(function () {
							return db.accounts.list({ address: account.address }).then(function (result) {
