...
const bootstrapSwagger = rewire(
	'../../../../../../src/modules/http_api/init_steps/bootstrap_swagger'
);
...
describe('init_steps/bootstrap_swagger', () => {
	...
	describe('middleware', () => {
		before(done => {
			middleware = bootstrapSwagger.__get__('middleware');
			validSendObject = {
				success: false,
		...
		describe('applyAPIAccessRules', () => {
			...
			beforeEach(done => {
				...
				bootstrapSwagger.__set__('checkIpInList', checkIpInListStub);
				done();
			});
			...
			it('should respond with error if api is not enabled', done => {
				...
				middleware.applyAPIAccessRules(
					validConfig,
					validReq,
					resMock,
					validNextSpy
