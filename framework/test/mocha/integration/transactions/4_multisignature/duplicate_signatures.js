...
describe('duplicate_signatures', () => {
	...
	localCommon.beforeBlock('lisk_functional_duplicate_signatures', lib => {
		...
		transactionPool = library.rewiredSubmodules.transactions.__get__(
	...
	describe('process multiple signatures from the same public key', () => {
		...
		it('should call a callback with error when there are multiple signatures', done => {
			...
			const lib = library.rewiredSubmodules.transactions.__get__('library');
			...
			describe('during multisignature account registration', () => {
				...
				it('should accept all signatures', done => {
					...
					async.parallel(
						async.reflectAll([
							parallelCb => {
								library.submodules.multisignatures.processSignature(
									signatures[0],
									parallelCb
								);
							},
