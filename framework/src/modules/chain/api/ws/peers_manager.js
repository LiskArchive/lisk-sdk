...
const connect = require('./rpc/connect');
const disconnect = require('./rpc/disconnect');
 ...
 */
function PeersManager(logger, system) {
	this.peers = {};
...
PeersManager.prototype.add = function(peer) {
	...
	if (peer.socket && peer.socket.active) {
		...
		connect(peer, this.logger, this.system.headers);
	}
	if (peer.nonce) {
