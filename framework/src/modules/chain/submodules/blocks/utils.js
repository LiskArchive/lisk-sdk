 ...
 */
class Utils {
	constructor(logger, account, block, storage, genesisBlock, transactions) {
		library = {
			logger,
			account,
			block,
	...
	readDbRows(rows) {
		...
		for (let i = 0, length = rows.length; i < length; i++) {
			...
			if (block) {
				...
				const transaction = self.modules.transactions.dbRead(rows[i]);
		...
		const blocks = rows.map(block => {
			...
			if (block) {
				...
				if (block.transactions) {
					block.transactions = self.modules.transactions.fromBlock(block);
				}
			}
			return block;
		});
	 ...
	 */
	// eslint-disable-next-line class-methods-use-this
	loadBlockByHeight(height, cb, tx) {
	 ...
	 */
	// eslint-disable-next-line class-methods-use-this
	_parseStorageObjToLegacyObj(block) {
		const parsedBlocks = [];
		let transactions = [{}];
	 ...
	 */
	// eslint-disable-next-line class-methods-use-this
	getBlockProgressLogger(transactionsCount, logsFrequency, msg) {
		/**
