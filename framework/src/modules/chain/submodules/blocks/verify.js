	...
	async.waterfall(
		...
		waterCbErr => {
			if (waterCbErr && waterCbErr.match(/Transaction is already confirmed/)) {
				...
				submodules.delegates.fork(block, 2);
...
__private.broadcastBlock = function(block, broadcast, cb) {
	if (broadcast) {
		try {
			...
			submodules.blocks.chain.broadcastReducedBlock(reducedBlock, broadcast);
		} catch (err) {
			return setImmediate(cb, err);
		}
	}
...
__private.validateBlockSlot = function(block, cb) {
	...
	submodules.delegates.validateBlockSlot(block, err => {
		if (err) {
			// Fork: Delegate does not match calculated slot
			submodules.delegates.fork(block, 3);
			return setImmediate(cb, err);
		}
		return setImmediate(cb);
	});
	...
	return async.series(
		{
			...
			applyBlock(seriesCb) {
				...
				submodules.blocks.chain.applyBlock(block, saveBlock, seriesCb);
			},
...
/**
 * Handle submodules initialization & components
 *
 * @param {Object} scope - Exposed submodules
 */
Verify.prototype.onBind = function(scope) {
	library.logger.trace('Blocks->Verify: Shared submodules bind.');
	components = {
