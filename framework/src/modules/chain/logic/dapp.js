...
class DApp {
	 ...
	 */
	// eslint-disable-next-line class-methods-use-this
	verify(transaction, sender, cb, tx) {
		...
		if (
			transaction.asset.dapp.tags &&
			transaction.asset.dapp.tags.length > 160
		) {
			return setImmediate(
				cb,
	 ...
	 */
	// eslint-disable-next-line class-methods-use-this
	applyConfirmed(transaction, block, sender, cb) {
	 ...
	 */
	// eslint-disable-next-line class-methods-use-this
	undoConfirmed(transaction, block, sender, cb) {
		return setImmediate(cb);
	}
	...
	objectNormalize(transaction) {
		...
		const report = __scope.schema.validate(transaction.asset.dapp, this.schema);
