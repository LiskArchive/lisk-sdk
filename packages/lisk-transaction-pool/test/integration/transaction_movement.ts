...
import { dummyCheckFunctionGenerator, dummyCheckerFunctionGenerator,  wrapExpectationInNextTick} from './common';
...
describe('transaction movement between queues', () => {
    ...
    const validateTransactionFunction = dummyCheckFunctionGenerator(['1']);
    const verifyTransactionFunction = dummyCheckFunctionGenerator(['2']);
    const processTransactionsFunction = dummyCheckFunctionGenerator(['3']);
    ...
    describe('validate received transactions', () => {
        ...
        beforeEach(async () => {
            ...
            transactionsToValidate = transactionPool.queues.received.peekUntil(returnTrueUntilLimit(configuration.receivedTransactionsLimitPerProcessing));
            const { passedTransactions, failedTransactions } = validateTransactionFunction(transactionsToValidate);
        ...
        it('should remove transactions from the received queue', async () => {
            return wrapExpectationInNextTick(() => {
                transactionsToValidate.forEach(transaction => {
                    expect(transactionPool.queues.received.exists(transaction)).to.equal(false);
                });
            });
        });
        ...
        it('should move valid transactions to the validated queue', async () => {
            return wrapExpectationInNextTick(() => {
                ...
                expect(transactionPool.queues.validated.size()).to.equal(validTransactions.length);
            });
        });
        ...
        describe('from validated to the verified and pending queues', () => {
            ...
            beforeEach(async () => {
                ...
                const { passedTransactions, failedTransactions } = verifyTransactionFunction(transactionsToVerify);
            ...
            it('should remove transactions from the validated queue', async () => {
                return wrapExpectationInNextTick(() => {
                    transactionsToVerify.forEach(transaction => {
                        expect(transactionPool.queues.validated.exists(transaction)).to.equal(false);
                    });
                });
            });
            ...
            it('should move verifiable transactions to the verfied queue', async () => {
                return wrapExpectationInNextTick(() => {
                    ...
                    expect(transactionPool.queues.verified.size()).to.equal(verifiableTransactions.length);
                });
            });
            ...
            describe('from pending and verified to the ready queue', () => {
                ...
                beforeEach(async () => {
                    ...
                    const { passedTransactions, failedTransactions } = processTransactionsFunction(transactionsToProcess);
                ...
                it('should remove transactions from the verified queue', async () => {
                    return wrapExpectationInNextTick(() => {
                        transactionsToProcess.forEach(transaction => {
                            expect(transactionPool.queues.verified.exists(transaction)).to.equal(false);
                        });
                    });
                });
                ...
                it('should move processable transactions to the ready queue', async () => {
                    return wrapExpectationInNextTick(() => {
                        ...
                        expect(transactionPool.queues.ready.size()).to.equal(processableTransactions.length);
                    });
                });
                ...
                it('should keep the transactions after running the job twice', async () => {
                    ...
                    await wrapExpectationInNextTick(() => {
                        transactionsInReadyQueue = transactionPool.queues.ready.transactions;
