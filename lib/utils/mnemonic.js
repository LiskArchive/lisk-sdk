...
function generateEmptyBytes(length) {
    return Array.apply(null, Array(length)).map(x => 0);
};
...
function generateSeed() {
    return generateEmptyBytes(16).map(randomByte).map(appendZero);
}
...
function seedToHex(seed) {
    return new Buffer(seed.join(''), 'hex');
}
...
function isValid(mnemonic) {
  ...
  var expected_hash_bits = entropyChecksum(buf);
