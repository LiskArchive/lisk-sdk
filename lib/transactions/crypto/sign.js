...
function signMessageWithTwoSecrets (message, secret, secondSecret) {
	var msg = naclInstance.encode_utf8(message);
	var keypair = keys.getRawPrivateAndPublicKeyFromSecret(secret);
	var secondKeypair = keys.getRawPrivateAndPublicKeyFromSecret(secondSecret);

	var signedMessage = naclInstance.crypto_sign(msg, keypair.privateKey);
	var doubleSignedMessage =  naclInstance.crypto_sign(signedMessage, secondKeypair.privateKey);
...
function verifyMessageWithTwoPublicKeys (signedMessage, publicKey, secondPublicKey) {
	...
	if (publicKeyBytes.length !== 32 || secondPublicKeyBytes.length !== 32) {
	...
	if (openSignature) {
		...
		if(openSecondSignature) {
			...
			throw new Error('Invalid signature publicKey2 combination, cannot verify message');
		}

	} else {
		throw new Error('Invalid signature publicKey1 combination, cannot verify message');
	}
}
