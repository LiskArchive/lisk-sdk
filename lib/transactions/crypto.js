...
function getTransactionBytes (transaction) {
	...
	function isDelegateTransaction () {
		return {
			assetSize: Buffer.from(transaction.asset.delegate.username, 'utf8').length,
			assetBytes: Buffer.from(transaction.asset.delegate.username, 'utf8')
		}
	}

	function isVoteTransaction () {
		var voteTransactionBytes = (new Buffer(transaction.asset.votes.join(''), 'utf8') || null);
		return {
			assetBytes: (new Buffer(transaction.asset.votes.join(''), 'utf8') || null),
	...
	function isDappTransaction () {
		return {
			...
			assetSize: getDAppBytes(transaction.asset.dapp).length
		}
	}
	...
	function isDappTransferTransaction () {
		...
		var dappBuffer = new Buffer(transaction.asset.dapptransfer.dappid, 'utf8');
