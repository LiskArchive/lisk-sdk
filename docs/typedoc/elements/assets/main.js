							...
							for (var h = s[l], m = 0; m < u; m++) {
								...
								if (v == h || h == '*') {
									...
									v in r.output.edges
										...
										: ((p = new t.TokenSet()), (p.final = E), (r.output.edges[v] = p)),
						...
						for (var b = 0; b < h.length; b++) {
							...
							for (var f = 0; f < this.metadataWhitelist.length; f++) {
								var S = this.metadataWhitelist[f],
									w = y.metadata[S];
								this.invertedIndex[y][o][r][S] == null && (this.invertedIndex[y][o][r][S] = []),
					...
					for (
						...
						for (
							...
							i[p] === void 0
								...
								(k =
									(w * ((this._k1 + 1) * f)) /
									(this._k1 * (1 - this._b + this._b * (u / this.averageFieldLength[a])) + f)),
								(k *= b),
								(k *= y),
